...
//CapturedPieces[HashSet<Piece>]
//PossibleEnPassant[Piece]
//Xeque[bool]
//Methods:

//ChessMatch()
//Piece ExecuteMove(Position source, Position target)
//void MakesMove(Position source, Position target)
//bool IsXeque(Color color)
//bool TestXequeMate(Color color)
//void UndoMove(Position source, Position target, Piece capturedPiece)
//void ChangePlayer()
//void PutNewPiece(char column, int line, Piece piece)
//void PutPieces()
//Color Adversary(Color color)
//HashSet<Piece> GetCapturedPieces(Color color)
...
//ChessMatch(): Constructor that initializes the game state.
//ExecuteMove(Position source, Position target): Executes a move from the source position to the target position, updating the state of the game and returning the piece that was captured (if any).
//MakesMove(Position source, Position target): Makes a move from the source position to the target position, checking for legality and updating the state of the game accordingly.
//UndoMove(Position source, Position target, Piece captured): Undoes a move from the source position to the target position, restoring the previous state of the game. The captured parameter should be the piece that was captured (if any).
//IsXeque(Color color): Checks if the given color is in check.
//TestXequeMate(Color color): Checks if the given color is in checkmate.
//ChangePlayer(): Changes the current player to the other color.